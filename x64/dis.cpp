...
void
InstructionTable::CopyTable (const ByteMnemonic bm[], InstructionType type)
{
...
void
InstructionTable::SetTableRange (InstructionType type, byte start, byte end,
                                 bool byte_size, const char *mnem)
{
...
void
InstructionTable::AddJumpConditionalShort ()
{
...
{
public:
  DisassemblerX64 (dis_client *client,
                   UnimplementedOpcodeAction unimplemented_action
                   = ABORT_ON_UNIMPLEMENTED_OPCODE)
      : client_ (client), tmp_buffer_pos_ (0),
        abort_on_unimplemented_ (unimplemented_action
                                 == ABORT_ON_UNIMPLEMENTED_OPCODE),
        rex_ (0), operand_size_ (0), group_1_prefix_ (0), vex_byte0_ (0),
        vex_byte1_ (0), vex_byte2_ (0), byte_size_operand_ (false),
        instruction_table_ (&instruction_table)
  {
    tmp_buffer_[0] = '\0';
  }
  ...
  int InstructionDecode (byte *start);
...
private:
  ...
  const char *
  NameOfCPURegister (int reg) const
  {
    if (0 <= reg && reg < 16)
      return cpu_regs[reg];
    return "noreg";
  ...
  const char *
  NameOfByteCPURegister (int reg) const
  {
    if (0 <= reg && reg < 16)
  ...
  const char *
  NameOfXMMRegister (int reg) const
  {
    if (0 <= reg && reg < 16)
      return xmm_regs[reg];
    return "noxmmreg";
  ...
  void
  get_modrm (byte data, int *mod, int *regop, int *rm)
  {
    *mod = (data >> 6) & 3;
  ...
  void
  get_sib (byte data, int *scale, int *index, int *base)
  {
    *scale = (data >> 6) & 3;
...
void
DisassemblerX64::AppendToBuffer (const char *format, ...)
{
  char *buf = tmp_buffer_ + tmp_buffer_pos_;
  va_list args;
  va_start (args, format);
  int result = vsnprintf (buf, 128 - tmp_buffer_pos_, format, args);
  va_end (args);
  tmp_buffer_pos_ += result;
}

int
DisassemblerX64::PrintRightOperandHelper (
    byte *modrmp, RegisterNameMapping direct_register_name)
{
  int mod, regop, rm;
  get_modrm (*modrmp, &mod, &regop, &rm);
  RegisterNameMapping register_name
      = (mod == 3) ? direct_register_name
      ...
      if ((rm & 7) == 5)
      ...
      else if ((rm & 7) == 4)
        {
          ...
          if (index == 4 && (base & 7) == 4 && scale == 0 /*times_1*/)
            {
              // index == rsp means no index. Only use sib byte with no index
              // for
              // rsp and r12 base.
          ...
          else if (base == 5)
            {
              ...
              AppendToBuffer ("[%s*%d%s0x%x]", NameOfCPURegister (index),
                              1 << scale, disp < 0 ? "-" : "+",
                              disp < 0 ? -disp : disp);
              return 6;
            }
...
int
DisassemblerX64::PrintImmediate (byte *data, OperandSize size)
{
  int64_t value;
  int count;
  switch (size)
...
int
DisassemblerX64::PrintRightOperand (byte *modrmp)
{
  return PrintRightOperandHelper (modrmp, &DisassemblerX64::NameOfCPURegister);
}

int
DisassemblerX64::PrintRightByteOperand (byte *modrmp)
{
...
int
DisassemblerX64::PrintRightXMMOperand (byte *modrmp)
{
...
int
DisassemblerX64::PrintOperands (const char *mnem, OperandType op_order,
                                byte *data)
{
  byte modrm = *data;
  int mod, regop, rm;
  ...
  int advance = 0;
  const char *register_name = byte_size_operand_
                                  ? NameOfByteCPURegister (regop)
...
int
DisassemblerX64::PrintImmediateOp (byte *data)
{
  ...
  AppendToBuffer (",0x");
  OperandSize immediate_size = byte_size_immediate ? OPERAND_BYTE_SIZE
                                                   : operand_size ();
...
int
DisassemblerX64::F6F7Instruction (byte *data)
{
...
int
DisassemblerX64::ShiftInstruction (byte *data)
{
  byte op = *data & (~1);
  int count = 1;
...
// Returns number of bytes used, including *data.
int
DisassemblerX64::JumpShort (byte *data)
{
  DCHECK_EQ (0xEB, *data);
  byte b = *(data + 1);
...
// Returns number of bytes used, including *data.
int
DisassemblerX64::JumpConditional (byte *data)
{
  DCHECK_EQ (0x0F, *data);
...
// Returns number of bytes used, including *data.
int
DisassemblerX64::JumpConditionalShort (byte *data)
{
  byte cond = *data & 0x0F;
  byte b = *(data + 1);
  byte *dest = data + static_cast<int8_t> (b) + 2;
...
// Returns number of bytes used, including *data.
int
DisassemblerX64::SetCC (byte *data)
{
  DCHECK_EQ (0x0F, *data);
  byte cond = *(data + 1) & 0x0F;
  const char *mnem = conditional_code_suffix[cond];
...
int
DisassemblerX64::AVXInstruction (byte *data)
{
  byte opcode = *data;
  byte *current = data + 1;
  if (vex_66 () && vex_0f38 ())
...
// Returns number of bytes used, including *data.
int
DisassemblerX64::FPUInstruction (byte *data)
{
...
int
DisassemblerX64::MemoryFPUInstruction (int escape_opcode, int modrm_byte,
                                       byte *modrm_start)
{
  const char *mnem = "?";
...
int
DisassemblerX64::RegisterFPUInstruction (int escape_opcode, byte modrm_byte)
{
...
int
DisassemblerX64::TwoByteOpcodeInstruction (byte *data)
{
  byte opcode = *(data + 1);
  byte *current = data + 2;
  ...
  if (operand_size_ == 0x66)
    ...
    {
      const char *const pseudo_op[] = {
        "rcpps",    "andps",    "andnps", "orps",  "xorps", "addps", "mulps",
        "cvtps2pd", "cvtdq2ps", "subps",  "minps", "divps", "maxps",
      };
      int mod, regop, rm;
      get_modrm (*current, &mod, &regop, &rm);
      AppendToBuffer ("%s %s,", pseudo_op[opcode - 0x53],
                      NameOfXMMRegister (regop));
      current += PrintRightXMMOperand (current);
    }
  else if (opcode == 0xC6)
  ...
  else if (opcode == 0xBE || opcode == 0xBF || opcode == 0xB6 || opcode == 0xB7
           || opcode == 0xAF)
    {
...
const char *
DisassemblerX64::TwoByteMnemonic (byte opcode)
{
  switch (opcode)
    {
    case 0x1F:
...
int
DisassemblerX64::InstructionDecode (byte *instr)
{
  tmp_buffer_pos_ = 0; // starting to write as position 0
  byte *data = reinterpret_cast<byte *> (instr);
  ...
  while (true)
    {
      current = *data;
      if (current == OPERAND_SIZE_OVERRIDE_PREFIX)
        { // Group 3 prefix.
        ...
        case 0xFF:
          {
            ...
            AppendToBuffer (((regop <= 1) ? "%s%c " : "%s "), mnem,
                            operand_size_code ());
            data += PrintRightOperand (data);
          }
          break;
            ...
            case OPERAND_DOUBLEWORD_SIZE:
              {
                const char *memory_location
                    = NameOfAddress (reinterpret_cast<byte *> (
                        *reinterpret_cast<int32_t *> (data + 1)));
                if (*data == 0xA1)
                  { // Opcode 0xA1
        ...
        case 0xA9:
          {
            ...
            AppendToBuffer ("test%c rax,0x%"
                            "08"
                            "x",
                            operand_size_code (), value);
            break;
          }
  ...
  client_->on_instr (tmp_buffer_);
  return instr_len;
}

//------------------------------------------------------------------------------

Disassembler::Disassembler (dis_client *client) : client_ (client) {}
...
{
  DisassemblerX64 d (client_, CONTINUE_ON_UNIMPLEMENTED_OPCODE);
  return d.InstructionDecode (reinterpret_cast<byte *> (start));
}
