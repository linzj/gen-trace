...
protected:
  ...
  virtual dis_client *new_backedge_check_client(intptr_t base, intptr_t hookend) = 0;
  virtual char * template_start() = 0;
  virtual char * template_ret_start() = 0;
  virtual char * template_end() = 0;
  virtual int max_tempoline_insert_space () = 0;
  virtual bool check_jump_dist (intptr_t target_code_point, intptr_t trampoline_code_start) = 0;
  virtual void flush_code (void *code_start, int len) = 0;
private:
bool
  check_for_back_edge (disassembler *, char *start, char *hook_end, char *code_end);
};
